...
- [x] Six tabs: **Appointments** (CalendarDays icon) · **Guests** (Users icon) · **Tasks** (CheckSquare icon) · **Payments** (CreditCard icon) · **Timeline** (Clock icon) · **More** (Menu icon)
  - Appointments, Guests, Tasks, Payments, Timeline: direct `<Link>` navigation
  - More: opens the sidebar (calls `setOpen(true)` via a context or prop)
  - Role-based visibility: Appointments and Payments visible to ADMIN + VIEWER only; Guests and Timeline visible to all; Tasks visible to all roles
...
**Commit: `d0c3129`** (initial), **`a743548`** (updated tabs to Appts/Guests/Tasks/Payments/More), **2026-04-05** (added Timeline tab)
