...
export async function DELETE(req: NextRequest, { params }: Params) {
  try {
    ...
    const count = await withTenantContext(weddingId, (tx) =>
      tx.timelineEvent.count({ where: { categoryId: id, weddingId } })
    );
    if (count > 0 && !force) {
      return NextResponse.json(
        { error: `${count} event${count === 1 ? "" : "s"} use this category`, count },
        { status: 409 }
      );
    }
    ...
    await withTenantContext(weddingId, async (tx) => {
      ...
      await tx.timelineCategory.delete({ where: { id, weddingId } });
    });
