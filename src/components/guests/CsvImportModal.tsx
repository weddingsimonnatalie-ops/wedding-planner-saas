...
export function CsvImportModal({ onClose, onImported }: Props) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error ?? "Parse error"); setLoading(false); return; }
  ...
  async function handleConfirm() {
    ...
    try {
      const res = await fetch("/api/guests/import", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ csv, confirm: true, duplicateActions: dupActions }),
      });
      const data = await res.json();
          ...
          {!preview && !result && (
            <div>
              <p className="text-sm text-gray-500 mb-4">
                Upload a CSV with columns: <strong>First Name, Last Name</strong> (required), and optionally
                Email, Phone, Group Name, Is Child (y/n), Ceremony (y/n), Reception (y/n), Afterparty (y/n), Notes.
              </p>
            ...
            <div className="space-y-5">
              ...
              <div className="flex flex-wrap gap-3 text-sm">
                <span className="flex items-center gap-1.5 text-green-600">
                  ...
                  {newRows.length} new guest{newRows.length !== 1 ? "s" : ""} — will be created
                </span>
                {dupRows.length > 0 && (
                    ...
                    <table className="w-full text-xs">
                      <thead className="bg-green-50">
                        <tr>
                          ...
                          <th className="text-left px-3 py-2 font-medium text-gray-600">Events</th>
                        </tr>
                      </thead>
                        ...
                        {newRows.map((row) => (
                          <tr key={row._line}>
                            <td className="px-3 py-2">
                              ...
                              {row.isChild && <span className="ml-1 text-gray-400">(child)</span>}
                            </td>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 sm:divide-x divide-amber-200">
                          ...
                          <div className="p-3 bg-amber-50">
                            ...
                            <p className="text-sm font-medium text-gray-900">
                              {row.firstName} {row.lastName}
                              {row.isChild && <span className="ml-1 text-xs text-gray-400">(child)</span>}
                            </p>
                          ...
                          <div className="p-3 bg-white">
                            ...
                            {row.existingGuest && (
                              <>
                                <p className="text-sm font-medium text-gray-900">
                                  {row.existingGuest.firstName} {row.existingGuest.lastName}
                                  {row.existingGuest.isChild && <span className="ml-1 text-xs text-gray-400">(child)</span>}
                                </p>
                                {row.existingGuest.email && <p className="text-xs text-gray-500">{row.existingGuest.email}</p>}
                                {row.existingGuest.phone && <p className="text-xs text-gray-500">{row.existingGuest.phone}</p>}
                                {row.existingGuest.groupName && <p className="text-xs text-gray-500">{row.existingGuest.groupName}</p>}
                                <p className="text-xs text-gray-500 mt-1">
                                  Events: {eventLabel(
                                ...
                                <p className="text-xs mt-1">
                                  ...
                                  <span className="font-medium">{row.existingGuest.rsvpStatus}</span>
                                </p>
                                {row.existingGuest.notes && (
                                  <p className="text-xs text-gray-400 mt-1 italic">{row.existingGuest.notes}</p>
                                )}
                              </>
                            )}
                          </div>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {errorRows.map((row) => (
                          <tr key={row._line} className="bg-red-50/50">
                            ...
                            <td className="px-3 py-2">{row.firstName || row.lastName ? `${row.firstName} ${row.lastName}`.trim() : "—"}</td>
          ...
          {result && (
            <div className="text-center py-8">
              ...
              <div className="mt-3 space-y-1 text-sm text-gray-600">
                {result.created > 0 && (
                  <p className="text-green-600">Created: {result.created} new guest{result.created !== 1 ? "s" : ""}</p>
                )}
                {result.updated > 0 && (
                  <p className="text-amber-600">Updated: {result.updated} existing guest{result.updated !== 1 ? "s" : ""}</p>
                )}
                {result.skipped > 0 && (
          ...
          {!result ? (
            <>
              ...
              {preview && actionableCount > 0 && (
                ...
                >
                  {loading ? "Importing…" : `Import ${actionableCount} guest${actionableCount !== 1 ? "s" : ""}`}
                </button>
              )}
            </>
          ) : (
